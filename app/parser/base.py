"""Shared parsing primitives for structured log extraction."""
...
class BaseLogParser:
    """Base class for file-level parsers with defensive helper methods."""
    ...
    _LEVEL_PATTERN = re.compile(
        r"\b(DEBUG|INFO|INFORMATION|WARNING|WARN|ERROR|ERR|CRITICAL|NOTICE|ALERT|EMERGENCY)\b",
        re.IGNORECASE,
    )
    _IP_PATTERN = re.compile(r"\b(?:\d{1,3}\.){3}\d{1,3}\b")

    def can_parse_file(self, raw_log: RawLogFile) -> int:
        """Return a confidence score for whether this parser fits the file."""
        return 0
    ...
    def parse_file(self, raw_log: RawLogFile) -> list[ParsedLogEntry]:
        """Parse one raw log file line by line."""
        entries: list[ParsedLogEntry] = []
    ...
    def _build_entry(
        ...
        """Create a parsed entry while applying shared normalization."""
        return ParsedLogEntry(
            ...
            ip_address=ip_address or self._extract_ip(raw_line),
    ...
    def _parse_datetime(self, value: str, *formats: str) -> datetime | None:
        """Parse a datetime value using the first matching format."""
        for fmt in formats:
            try:
    ...
    def _parse_syslog_timestamp(self, value: str) -> datetime | None:
        ...
        parsed = self._parse_datetime(value, "%b %d %H:%M:%S")
        if parsed is None:
            return None
    ...
    def _normalize_level(self, value: str | None) -> str:
    ...
    def _level_from_status_code(self, status_code: int | None) -> str:
        """Map HTTP response status families to practical levels."""
